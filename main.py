...
def train_obj():
    num_freqs = 6
    obj_path = pathlib.Path("data/sdf/dragon.obj")
    batch_size = 512
    dev = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

    train_dataset = OBJDataset(str(obj_path), num_samples=2**22, tsdf_min=-0.25, tsdf_max=0.25, voxel_resolution=512)
    val_dataset = OBJDataset(str(obj_path), num_samples=2**16, tsdf_min=-0.25, tsdf_max=0.25, voxel_resolution=512)
    ...
    out_path = obj_path.parent / (obj_path.stem + "_pred.ply")
    predict_mesh(val_dataset, model, str(out_path))


def train_sdf():
    num_freqs = 6
    sdf_path = pathlib.Path("data/sdf/bunny_nonormals.sdf")
    batch_size = 256
    dev = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    ...
    trainer.fit(model, train_dataloaders=train_dataloader, val_dataloaders=val_dataloader)
    out_path = sdf_path.parent / (sdf_path.stem + "_pred.ply")
    predict_mesh(val_dataset, model, str(out_path))


def train_super_res():
    num_freqs = 6
    img_path = pathlib.Path("data/image/yosemite.jpg")
    batch_size = 2**15
    dev = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

    train_dataset = ImageDataset(str(img_path))
    model = SuperResolutionNGPModel(pos_enc_freqs=num_freqs)
    # model = SuperResolutionNGPModel.load_from_checkpoint("lightning_logs/version_39/checkpoints/epoch=19-step=39999.ckpt")
    model.to(dev)
    ...
    out_path = img_path.parent / (img_path.stem + "_pred.jpg")
